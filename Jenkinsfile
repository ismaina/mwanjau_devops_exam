try {
    node {
        ...
        stage('Build docker image') {
            sh "docker build -t mwanjau_devops_exam:latest ."
        }
        stage('Docker login to hub and push the image') {
            sh "docker login -u 'mwanjau' -p 'mwanjau@123' "
            sh "docker tag mwanjau_devops_exam:latest mwanjau/mwanjau_devops_exam:latest"
            sh "docker push mwanjau/mwanjau_devops_exam:latest"
        }
